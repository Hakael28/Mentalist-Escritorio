...
public class RegistroUsuarioPanel extends javax.swing.JPanel {
    ...
    private void guardarBTNActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarBTNActionPerformed
        try {
            // Validación básica
            if (SetUsuario.getText().trim().isEmpty() || SetNombre.getText().trim().isEmpty() || correoTFX.getText().trim().isEmpty()) {
                AlertaUtils.mostrarAdvertencia("Todos los campos deben estar llenos.");
                return;
            }

            // Construcción del DTO
            UsuarioDTO dto = new UsuarioDTO();
            dto.setIdUsuario(Integer.parseInt(SetID.getText()));
            dto.setUsuario(SetUsuario.getText().trim());
            dto.setNombre(SetNombre.getText().trim());

            Object item = SetRol.getSelectedItem();
            Rol rolSeleccionado = Rol.valueOf(item.toString().trim().toUpperCase());
            dto.setRol(rolSeleccionado);

            dto.setCorreo(correoTFX.getText().trim());
            dto.setTelefono(telefonoTXT.getText().trim());


            // Actualizar usuario (como admin)
            UsuarioDTO actualizado = UsuarioCliente.actualizarUsuarioComoAdmin(dto.getIdUsuario(), dto);

            AlertaUtils.mostrarExito("Usuario actualizado correctamente.");

            // Volver al panel UsuariosPanel
            UsuariosPanel panelUsuarios = new UsuariosPanel();
            JPanel contenedor = (JPanel) this.getParent();
            contenedor.removeAll();
            contenedor.setLayout(new BorderLayout());
            contenedor.add(panelUsuarios, BorderLayout.CENTER);
            contenedor.revalidate();
            contenedor.repaint();

        } catch (Exception e) {
            e.printStackTrace();
            AlertaUtils.mostrarError("Error al actualizar el usuario: " + e.getMessage());
        }
    }
}//GEN-LAST:event_guardarBTNActionPerforme
